...
class_name Retry_Charged_Projectile_Spell
...
func activate(current_caster:Player):
	if (can_fire):
		...
		current_projectile = this_spell.projectile_scene.instantiate()
		...
		current_projectile.initialize(this_spell.speed,projectile_sprite,damage,lifetime,size,true)
		#print(current_projectile.paused)

func fire():
	
	var charge_ratio = (charge_time-maxf(current_charge_time,0))/charge_time
	%Cooldown_Timer.wait_time=this_spell.cooldown*this_spell.cooldown_factor.sample_baked(charge_ratio)
