...
func activate(current_caster:Player):
	if (current_cooldown<0):
		...
		current_projectile.transform=get_parent().get_parent().transform
		current_projectile.move_dir=get_parent().get_parent().velocity.normalized()
		current_projectile.initialize(0,projectile_sprite,damage,lifetime,size,true)
